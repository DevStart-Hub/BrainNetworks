...
Look how far we've come!! In the first two sections we did two big things. First, we *described* brain networks: we measured their topology and asked whether it relates to early life stress. Then, we *grew* brain-like networks with GNMs: we found a tiny set of wiring rules that reproduces the brain's architecture.

And here's something you might not have noticed: through all of that, **everything came back to one object**. The connectivity matrix! That humble grid of numbers telling us *who connects to whom*. We plotted it, we thresholded it, we measured its topology, and with GNMs we even grew it from scratch. It has quietly been the main character of this whole journey.

So before meeting our shiny new tool, let's start from what we already have: that matrix.

## Not another matrix that just sits there

For all its powers, our connectivity matrix has one big limitation: it's a **map**, and a map doesn't *do* anything... It just gives us a very informative but completely **static** picture.
...
But hold on — we keep saying the units are "active" and that activity "flows". What does that actually mean, and where does the activity even come from? To really understand what $W_{rec}$ does, let's follow the activity through the network one step at a time. It all starts with the input.
