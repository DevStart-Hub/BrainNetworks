...
Each brain network is a 100×100 weighted connectivity matrix (the cell values are streamline counts, reflecting *how strong* each connection is), just like the one we saw in the previous tutorial. The full dataset is stored as a single array with shape `[20, 100, 100]`. Later, we'll threshold these weighted networks to a common density (which also turns them into simple binary "connected / not-connected" maps) before modelling.
