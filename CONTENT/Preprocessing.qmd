...
What just happened? `nx.from_numpy_array()` reads our connectivity matrix and creates a **Graph object**. Technically, each row/column becomes a node, and each non-zero cell becomes an edge. But practically, this is just the only format that NetworkX likes, and so we'll respect its preferences!!! And once we do, we can then use all of `networkx`'s built-in functions on it!
...
Now let's compute the density for all 20 brains in a loop:
...
We also highlighted the minimum density: In this case 0.1822. Any idea why!?? If not... keep on reading!
...
But what density should we target? We use the **minimum density** across all brains. Why? Because we can only *remove* connections (we can't invent new ones!), so the sparsest brain sets the limit for everyone. That's why we highlighted it earlier!!

We'll use a handy function from the `netneurotools` package, `threshold_network`. Given a network, it tells us **which connections to keep**: it hands back a grid of 0s and 1s, where a **1** marks one of the strongest connections (keep it) and a **0** marks a weak one (drop it).

That grid is just a *mask*. To actually thin out a brain, we **apply the mask** to our weighted matrix by multiplying the two together: wherever the mask is 0, the connection is wiped to zero; wherever it's 1, the connection keeps its original strength.
...
for i in range(num_brains):
    ...
    brains_thresholded[i] = keep_mask * brains[i]                    # discard the weak ones, keep the weights
```

The `retain` argument tells the function what proportion of connections to keep, and since we set it to the minimum density, every brain now has exactly the same number of connections. We are on a level playing field!
...
3.  **Add a minimal number of connections** to reconnect the disconnected parts of the network. This is a bit hacky, but the only way to keep all the data!!!
:::
